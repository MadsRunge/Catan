...
import { botChooseRobberHex, botChooseStealTarget, botTurn } from '../bot';
...
describe('Bot', () => {
  describe('botChooseRobberHex', () => {
    it('should choose a hex that does not have the robber', () => {
      ...
      const state: GameState = { players: [], currentPlayerIndex: 0, hexes, robberHexId: 1, devCardDeck: [], winnerId: null };
  ...
  describe('botTurn', () => {
    it('should play a knight card if available', () => {
      ...
      const player2 = { ...createMockPlayer(2), resources: { wood: 1 } };
      const hexes = [createMockHex(1), createMockHex(2)];
